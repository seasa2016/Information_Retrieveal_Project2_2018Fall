...
class RNNC(nn.Module):
    def __init__(self,token,args):
        ...
        self.ner_emb = nn.Linear(args.word_dim,len(token['nodes']))
    ...
    def forward(self,data,data_len,data_ner,data_point):
        ...
        def feat_extract(output,length,mask):
            ...
            for j in range(2):
                temp = [torch.cat([ output[i][ length[i][j][1]-1 ][:self.hidden_size] , 
                                            output[i][ length[i][j][0] ][self.hidden_size:]] , dim=-1 ) for i in range(length.shape[0])]
        ...
        query_result = feat_extract(query_res,data_point,mask)

        output = self.dense_1(query_result)
        output = self.act_1(output)
        output = self.dense_2(output)
        
        output_ner = self.ner_emb(query_res)

        return output,output_ner
