...
def build_info(dataset):
    ...
    for data in dataset:
        ...
        nodes = data['nodes']
        for node in nodes:
            nodes_info['node_class'].extend(list(node[1].keys()))  
...
lexical_feat = [np.array(list(train_noun1[i][0]) + list(train_noun2[i][0]) + \
                         ...
                         list(train_RT[i][0])) for i in range(train_data_size)] # shape: 16959 x (256 x 6)
...
model_trained = cnn_model.fit_generator(generator = data_loader(lexical_feat[:int(train_data_size*0.8)], word_position_feat[:int(train_data_size*0.8)], y_train[:int(train_data_size*0.8)], batch_size),                  
                                        validation_data = data_loader(lexical_feat[int(train_data_size*0.8):], word_position_feat[int(train_data_size*0.8):], y_train[int(train_data_size*0.8):], batch_size), 
                                        steps_per_epoch = int(train_data_size*0.8 / batch_size),
                                        validation_steps = int(train_data_size*0.2 / batch_size * 5),
                                        epochs = epochs)
'''
model_trained = cnn_model.fit_generator(generator = data_loader(lexical_feat[:int(train_data_size*0.8)], word_position_feat[:int(train_data_size*0.8)], y_train[:int(train_data_size*0.8)], batch_size),                  
                                        ...
                                        steps_per_epoch = int(train_data_size*0.8 / batch_size),
                                        epochs = epochs)
cnn_model.save('savings/edge_detection_cnn.h5')
